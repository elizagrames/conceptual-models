...
space <- c("canopy cover", "forest", "woodland", 
           "woodlot", "broadleaf", "coniferous", 'deciduous')

# Functions --------------------------------------------------------------------

# From litsearchr (Grames et al. 2019)
...
load("./data/search_results_deduped.rda")

texts <- tolower(paste(search_results$title, search_results$abstract, 
                       search_results$keywords, search_results$taxa_notes, 
                       search_results$concepts))

# Stem lists of terms to search for word forms
...
# Tag articles by search terms -------------------------------------------------

# Dictionary of search terms
search_dictionary <-
  list(
    outcome = append(time, outcome),
    population = population,
    intervention = intervention,
    space = space
  )

# Check for presence of search terms in title, abstract, and keywords
search_info <- topictagger::tag_strictly(texts,
                                         scheme = search_dictionary,
                                         allow_multiple = TRUE)

# Presence only -- does not matter if multiple terms matched within a group
search_info[search_info>0] <- 1

# Did all categories get matched?
all_cats <- rowSums(search_info)
missing_terms <- all_cats!=4

# Tag study locations for exclusion criteria -----------------------------------

# Set up a georeference database based on cities and geopolitical regions
geodat <- read.csv("./data/worldcities.csv")

# Remove cities and regions that are common words in English
...
undata <- read.csv("./data/UN_countries.csv")
geodat$continents <-
...
# Clean up extra geotagging info
rm(l2,l3,l4, time, space, outcome, population, 
   twocountries, intervention, checkgeo, geodat, undata)
...
# Tag study focal taxa for exclusion criteria ----------------------------------

# Set up a dictionary based on bird phylogeny list
...
# We only want to keep passerines
...
retain <-
  which(
    missing_terms == FALSE &
      continents %in% c("Northern America", NA) &
      (passerines > 0 | rowSums(taxa_tags) == 0)
  )
